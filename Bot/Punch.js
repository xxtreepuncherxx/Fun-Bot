 ...
 * @Admin :                - xXTreePuncherXx  (Owner. on PunchCraft MC)
 *
 */
...
toSave = {};
toSave.settings = PunchBot.settings;
toSave.moderators = PunchBot.moderators;
 
PunchBot.misc.version = "2.0.1";
PunchBot.misc.ready = true;
...
// Filterng Chat
PunchBot.filters.beggerWords = new Array();
PunchBot.filters.commandWords = new Array();

// Bot's settings
PunchBot.settings.songLimit = 10; 
PunchBot.settings.cooldown = 10; 
PunchBot.settings.staffMeansAccess = true;
PunchBot.settings.historyFilter = true;
PunchBot.settings.beggerFilter = true;
PunchBot.settings.commandFilter = true;
PunchBot.settings.interactive = true;
PunchBot.settings.ruleSkip = true;
PunchBot.settings.removedFilter = true;

// Admins ID
PunchBot.admins = ["50aeaeb6c3b97a2cb4c25bd2","50dc4482877b922314e34347"];
...
// Filter Keywords
PunchBot.filters.beggerWords = ["fanme","fan me","fan4fan","fan 4 fan","fan pls","fans please","need fan","more fan","fan back","give me fans","gimme fans","need fan"];
PunchBot.filters.commandWords = [".say",".test",".ping",".marco",".reward",".add",".addsong",".flipcoin",".catfact",".dogfact",".hug",".8ball",".fortune",".songlink",".download",".help",".whywoot",".whymeh",".props",".votes",".woot",".meh",".version",".userstats @",".mystats",".source",".roomstats",".roomstats2",".register",".join",".leave",".roll"];


// Fun misc
PunchBot.misc.tacos = ["blunt","kush","Chemo","Locoweed","marijuana","Ganja"];
PunchBot.misc.cookie = ["a chocolate chip cookie", "a sugar cookie", "an oatmeal raisin cookie", "a 'special' brownie", "an animal cracker", "a scooby snack", "a blueberry muffin", "a cupcake","Strawberry Sunday", "Chocolate Chip Icecream Cone", "Cookie Dough Triple Scoop ", "Mint Chocolate Chip Icecream Cone", "Chocolate Icecream Sunday", "Banana Split with Whipped Cream", "Vanilla Icecream Cone with Sprinkles ", "Bubblegum Flavored Popcicle"];
PunchBot.misc.ball = [
" It is certain",
" It is decidedly so",
" Without a doubt",
...
PunchBot.misc.ht = ["My magic coins says: Tails", "My magic coin says: Heads"];
...
PunchBot.pubVars.skipOnExceed;
PunchBot.pubVars.command = false;
...
PunchBot.skip = function(){
API.moderateForceSkip();
};

PunchBot.unhook = function(){
...
botMethods.load = function(){
    toSave = JSON.parse(localStorage.getItem("PunchBotSave"));
    PunchBot.settings = toSave.settings;
    ruleSkip = toSave.ruleSkip;
};
 
botMethods.save = function(){localStorage.setItem("PunchBotSave", JSON.stringify(toSave))};
 
botMethods.loadStorage = function(){
    if(localStorage.getItem("PunchBotSave") !== null){
        botMethods.load();
    }else{
        botMethods.save();
    }
...
function chatMe(msg)
{
    ...
    API.on(API.CHAT, function(data){
        if(data.message.indexOf('.') === 0){
            ...
            if(PunchBot.misc.ready || PunchBot.admins.indexOf(fromID) > -1 || API.getUser(data.fromID).permission > 1 || API.getUser(fromID).permission < 2){
                ...
                case "say":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1 || typeof command[1] === "undefined"){
                ...
                case "grab":
                   ...
                   case "props":
                        if(API.getUser(fromID).permission < 2 || API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1){
                        if(typeof command[1] === "undefined"){
                   ...
                   case "votes":
                        ...
                        API.sendChat("Users vote:  :+1: " + API.getRoomScore().positive + " | :-1: " + API.getRoomScore().negative + " | :purple_heart: " + API.getRoomScore().curates);
                            PunchBot.misc.ready = false;
                            setTimeout(function(){ PunchBot.misc.ready = true; }, PunchBot.settings.cooldown * 1000);
                        }
                        break;
                        
                   case "version":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1){
                        API.sendChat("Bot Version "+ PunchBot.misc.version);
                            PunchBot.misc.ready = false;
                            setTimeout(function(){ PunchBot.misc.ready = true; }, PunchBot.settings.cooldown * 1000);
                        }else{
                           API.sendChat("This command requires staff members only!");
                   ...
                   case "source":
                       if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1){
                            API.sendChat("[Staff] Your bot code is located here: http://goo.gl/6V7t0d");
                        }else{
                           API.sendChat("This command requires staff members only!");
                        }
                        break;
                   ...
                   case "whywoot":
                        ...
                        if(PunchBot.admins.indexOf(fromID) > -1 || API.getUser(fromID).permission < 2){
                            PunchBot.misc.ready = false;
                            setTimeout(function(){ PunchBot.misc.ready = true; }, PunchBot.settings.cooldown * 1000);
                        }
                        break;
                   ...
                   case "creator":
                        if(API.getUser(fromID).permission < 2 || PunchBot.admins.indexOf(fromID) !== -1){
                           API.sendChat("This bot was created by: xXTreePuncherXx and ๖ۣۜĐل - ɴᴇᴏɴ - TFL");
                        }
                        break;
                       
                   case "beggerfilter":
                   case "bf":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1) PunchBot.settings.beggerFilter ? API.sendChat("Begger filter is enabled") : API.sendChat("Begger filter is disabled");
                        botMethods.save();
                        break;
                        
                   case "commandfilter":
                   case "cf":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1) PunchBot.settings.commandFilter ? API.sendChat("Commands filter is enabled") : API.sendChat("Commands filter is disabled");
                        botMethods.save();
                        break;    
                        
                   case "tbf":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1){
                            if(PunchBot.settings.beggerFilter){
                                PunchBot.settings.beggerFilter = false;
                   ...
                   case "tcf":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1){
                           if(PunchBot.settings.commandFilter){
                                PunchBot.settings.commandFilter = false;
                ...
                /*case "songLimit":
                       ...
                       PunchBot.settings.songLimit = command[1];
                          }
                       }
                       break;*/     
                   ...
                   case "status":
                        if(API.getUser(fromID).permission > 1 || PunchBot.admins.indexOf(fromID) > -1){
                            ...
                            response = response + " | Begger filter: "+ PunchBot.settings.beggerFilter;
                            response = response + " | History filter: "+ PunchBot.settings.historyFilter;
                            response = response + " | SongLimit: " + PunchBot.settings.songLimit + "m";
                            response = response + " | Cooldown: " + PunchBot.settings.cooldown + "s";
                            response = response + " | CPU Filter: "+ PunchBot.settings.removedFilter;
                  ...
                  case "fortune":
                        if(typeof command[1] == "undefined"){
                            ...
                            var randomFortune = Math.floor(Math.random() * PunchBot.misc.fortune.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat("@" + data.from + ","+ PunchBot.misc.fortune[randomFortune]);
                                    break;
                                case 1:
                                    API.sendChat("@" + data.from + ","+ PunchBot.misc.fortune[randomFortune]);
                                    break;
                 ...
                 case "roll":
                        if(typeof command[1] == "undefined"){
                            var crowd = API.getUsers();
                            var randomUser = Math.floor(Math.random() * crowd.length);
                            var randomRoll = Math.floor(Math.random() * PunchBot.misc.roll.length);
                            var randomSentence = Math.floor(Math.random() * 2);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat("@"+ data.from +" You rolled a "+ PunchBot.misc.roll2[randomRoll]);
                                    setTimeout(function(){
                                    document.getElementById("woot").click()
                                    }, 650);
                                    break;
                                case 1:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.roll[randomRoll]);
                                    break;
                            }
                        }else{
                            if(command[1].indexOf("@") === 0) command[1] = command[1].substring(1);
                 ...
                 case "8ball":
                        if(typeof command[1] == "undefined"){
                            var crowd = API.getUsers();
                            var randomUser = Math.floor(Math.random() * crowd.length);
                            var randomBall = Math.floor(Math.random() * PunchBot.misc.ball.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.ball[randomBall]);
                                    break;
                                case 1:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.ball[randomBall]);
                                    break;
                    ...
                    case "flipcoin":
                        if(typeof command[1] == "undefined"){
                            var crowd = API.getUsers();
                            var randomUser = Math.floor(Math.random() * crowd.length);
                            var randomHt = Math.floor(Math.random() * PunchBot.misc.ht.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat(PunchBot.misc.ht[randomHt]);
                                    break;
                                case 1:
                                    API.sendChat(PunchBot.misc.ht[randomHt]);
                                    break;
                    ...
                    case "reward":
                        if(typeof command[1] == "@"){
                            var crowd = API.getUsers();
                            var randomUser = Math.floor(Math.random() * crowd.length);
                            var randomCookie = Math.floor(Math.random() * PunchBot.misc.cookie.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat(crowd[randomUser].username+ ", @" + data.from + " has rewarded you with " + PunchBot.misc.cookie[randomCookie]+ ". Enjoy!");
                                    break;
                                case 1:
                                    API.sendChat(crowd[randomUser].username+ ", @" + data.from + " has rewarded you with " + PunchBot.misc.cookie[randomCookie]+ ". Enjoy!");
                                    break;
                        ...
                        if(typeof command[1] == "@") command[1] = command[1].substring(1);
                            ...
                            switch(randomSentence){
                                case 0:
                                    API.sendChat(command[1]+", "+ data.from +" has rewarded you with " + PunchBot.misc.cookie[randomCookie]+ ". Enjoy!");
                                    break;
                                case 1:
                                    API.sendChat(command[1]+", "+ data.from +" has rewarded you with " + PunchBot.misc.cookie[randomCookie]+ ". Enjoy!");
                                    break;
                 ...
                 case "dogfact":
                        if(typeof command[1] == "undefined"){
                            var crowd = API.getUsers();
                            var randomUser = Math.floor(Math.random() * crowd.length);
                            var randomDogfact = Math.floor(Math.random() * PunchBot.misc.dogfact.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.dogfact[randomDogfact]);
                                    break;
                                case 1:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.dogfact[randomDogfact]);
                                    break;
                    ...
                    case "catfact":
                        if(typeof command[1] == "undefined"){
                            var crowd = API.getUsers();
                            var randomUser = Math.floor(Math.random() * crowd.length);
                            var randomCatfact = Math.floor(Math.random() * PunchBot.misc.catfact.length);
                            var randomSentence = Math.floor(Math.random() * 1);
                            switch(randomSentence){
                                case 0:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.catfact[randomCatfact]);
                                    break;
                                case 1:
                                    API.sendChat("@" + data.from + ", "+ PunchBot.misc.catfact[randomCatfact]);
                                    break;
    ...
    API.on(API.CHAT, function(data){
        ...
        for(var i = 0; i < PunchBot.filters.beggerWords.length; i++){
            if(msg.indexOf(PunchBot.filters.beggerWords[i].toLowerCase()) > -1 && PunchBot.settings.beggerFilter){
            ...
            if(msg.indexOf(PunchBot.filters.commandWords[i].toLowerCase()) > -1 && PunchBot.settings.commandFilter){
               API.moderateDeleteChat(chatID);
            }
        }
    ...
    function DJ_ADVANCE(data){
        ...
        setTimeout(function(){
            if(typeof response === 'undefined' && data.media.format != 2 && PunchBot.settings.removedFilter){
    ...
    console.log("PunchBot-Script version "+ PunchBot.misc.version);
    ...
    API.sendChat('Punch-Bot version '+PunchBot.misc.version+' Activated!');
   }else{
    API.sendChat("This bot can only function at http://plug.dj/punchcraft/");
   };
